...
executeH p pc rb
  ...
  | instr == Out  = do p' <- output p (get p (mode a p rb (pc+1))); executeH p' (pc+2) rb
...
executeC p pc rb 
  ...
  | instr == Out  = do p' <- output p (get p (mode a p rb (pc+1))); executeC p' (pc+2) rb 
...
calculate op (a,b,c) p pc rb = let pos1  = mode a p rb (get p (pc+1)) 
                                   pos2  = mode b p rb (get p (pc+2)) 
                                   store = get p (pc+3)
                               in set p store (compute op pos1 pos2)

-- |Given a parameter mode, returns the immediate value of the position 
mode :: PMode -> Program -> Int -> Int -> Int
mode Pos p rb i = get p i
mode Imm p rb i = i 
mode Rel p rb i = get p (rb + i)
...
-- |Sets a given index in a program to a given instruction 
...
jumpOn p predicate (a,b,_) pc rb  
  | predicate pos1 = pos2
  | otherwise      = pc + 3
  where pos1 = mode a p rb (get p (pc+1))  
        pos2 = mode b p rb (get p (pc+2)) 

-- |Calculates position from relative base offset 
offset :: Program -> Parameters -> Int -> Int -> Int  
offset p (a,_,_) pc rb = let off = mode a p rb (get p (pc+1)) in off
...
executeI p pc rb
  ...
  | instr == Out  = do prompt p pc rb; p' <- output p (get p (mode a p rb (pc+1))); executeI p' (pc+2) rb
